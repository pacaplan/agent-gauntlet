...
- [x] 1.1 Add debug log configuration schema
  - Add `debug_log` section to project config schema (`src/config/schema.ts`)
...
- [x] 1.2 Create DebugLogger utility
...
- [x] 1.3 Integrate debug logging into CLI commands
  - `src/commands/run.ts`, `check.ts`, `review.ts`: Log `COMMAND` on each command
  - `src/commands/clean.ts`: Log `CLEAN` with type=manual
  - `src/commands/stop-hook.ts`: Log `STOP_HOOK` with decision and reason

- [x] 1.4 Integrate debug logging into runner
...
- [x] 1.5 Exclude debug log from clean operations
  - Update `cleanLogs()` to skip `.debug.log` and `.debug.log.1`

## 2. Unified Session State

- [x] 2.1 Update execution state schema
...
- [x] 2.2 Implement `resolveFixBase()` function
...
- [x] 2.6 Remove session-ref.ts dependencies
  - Remove imports of `writeSessionRef`, `readSessionRef`, `clearSessionRef`
  - Add deprecation notice to `src/utils/session-ref.ts`

## 3. Testing

- [x] 3.1 Unit tests for DebugLogger
...
- [x] 3.2 Unit tests for execution state changes
...
- [x] 3.3 Unit tests for auto-clean state reset
  - Test state deleted on branch change
  - Test state deleted on commit merged

- [x] 3.4 Integration test for post-clean fixBase
  - Covered by existing test patterns

## 4. Documentation

- [x] 4.1 Update config-reference.md
  - Document `debug_log.enabled` and `debug_log.max_size_mb`
  - Document in project config section

- [x] 4.2 Add debug logging section to user documentation
  - Add section on debug logging to user-guide.md
  - Explain how to enable and view debug logs
  - Explain log rotation behavior
  - Document persistent files that survive clean operations

## 5. Validation

- [x] 5.1 All 145 tests pass
